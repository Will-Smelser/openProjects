 ...
 */
public class CloudMessageServicer<T extends Serializable> {
    ...
    private final IMap<String, CloudMessageHandler<T>> handlers;
     ...
     */
    public CloudMessageServicer(final String group, final String name, final HazelcastInstance hcast, final CloudMessageHandler<T> handler){
        this.name = name;
        this.group = group;
        this.hcast = hcast;
    ...
    public Future<T> send(String to, T message) throws CloudMessageError {
        ...
        CloudMessageTask<T> task = new CloudMessageTask<>(cMessage, handler);

        //submit the message
        return executor.submit(task);
    }

    public T send(String to, T message, long timeout, TimeUnit unit) throws CloudMessageError, TimeoutException {
        Future<T> future = send(to, message);
        try {
     ...
     */
    private static class CloudMessageTask <T extends Serializable> implements Callable<T>, Serializable {

        private final CloudMessage<T> message;

        private final CloudMessageHandler<T> handler;

        public CloudMessageTask(CloudMessage<T> message, CloudMessageHandler<T> handler){
