...
var HashRouter = function () {
     ...
     * @param fn {Function} [optional] A function to call when a Path is matched.  Either triggered from
     * calling {@link this.eval(path)} or when "onhashchange" event is active.
     */
    ...
    this.start = function () {
        ...
        if ("onhashchange" in window.document.body) {
            window.onhashchange = function () {
                self.evaluate(window.location.hash);
            }
            self.evaluate(window.location.hash);
            return;
        }
